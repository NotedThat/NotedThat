...
2. `release-plz-pr` runs after (needs: release-plz-release). It opens or updates a release PR with the next version bump and aggregated `CHANGELOG.md` entries from all 11 crates.
...
4. Repeat for all 11 crates: `notedthat-core`, `notedthat-storage-s3`, `notedthat-storage-fs`, `notedthat-indexer`, `notedthat-write`, `notedthat-api-http`, `notedthat-webdav`, `notedthat-mcp`, `notedthat-server`, `notedthat-mcp-stdio`, `notedthat`
