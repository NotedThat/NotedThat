...
| `NOTEDTHAT_MCP_MAX_SESSIONS` | `--mcp-max-sessions` | positive integer | 256 | Most MCP sessions one process holds at once. A `POST` that would open another — one carrying no `Mcp-Session-Id` — is refused `503` with `Retry-After: 5` until a session ends or idles out. A **soft** bound: the count and the session's creation are not atomic, so concurrent `initialize`s can overshoot it by however many were in flight, and it is per process rather than per caller. Zero, a negative number or a non-number refuses startup. See [what a session costs](#what-a-session-costs) before raising it. |
