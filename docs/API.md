...
**Arguments**: `kb?` (array of slugs), `query` (string), `filters?` (object), `limit?` (u32). `filters` takes exactly the HTTP body's [`filter` fields](#post-apiv1knowledgebaseskb_slugsearch): `object_key_prefix`, `mime`, `concept_type`, `heading_path_prefix`, `updated_after`, `updated_before`, `tags` — AND-composed, each with a description in the tool's input schema. An unknown argument or filter key is `invalid_request` naming the key, as on the HTTP route; `filter` (singular) is the likeliest slip.
