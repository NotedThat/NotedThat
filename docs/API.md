...
| `reconcile` | Either backend's comparison of its storage against the index. On `fs`: at startup, after a directory-level change (a new or renamed folder's files arrive this way), or on a rescan. On `s3`: at startup, and on `POST …/index/reconcile` (D66) |
...
(D66). This is how the `s3` backend learns about objects written, changed or deleted by
