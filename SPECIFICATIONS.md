...
| D70 | S3 conditional writes are checked at startup | **On the `s3` backend each knowledge base's bucket is asked once at startup, before provisioning touches anything else, whether it enforces the preconditions NotedThat forwards on a `PUT`; a bucket that does not refuses startup unless `NOTEDTHAT_S3_ALLOW_UNENFORCED_CONDITIONAL_WRITES=true` accepts the risk.** NotedThat forwards `If-Match` and `If-None-Match` verbatim and adds no lock of its own (§8), so optimistic concurrency on every surface — `PUT`, `PATCH`, MCP `edit`/`append`/`replace`, WebDAV — is exactly as good as the backend's enforcement, and a backend that parses the headers and stores the object anyway (§8.1: Garage, SeaweedFS < 4.09) loses one of two concurrent writes with a `200` to both. Nothing in a normal request can tell, so the check asks directly: `S3Storage::check_conditional_writes` stores a scratch object at `.notedthat/conditional-write-probe-<uuid>` and overwrites it three times, because enforcement is a claim in **both** directions and asking only the half that cannot hold does not distinguish a backend that compares correctly from one that never matches at all: an `If-Match` naming the `ETag` the first `PUT` returned, which must be **stored**; the same `ETag` with one hex digit altered, which must be `412`; and `If-None-Match: *` over the object that now exists, also `412`. Altering the returned `ETag` rather than inventing a literal keeps the value syntactically valid, so a backend that rejects a malformed `ETag` with `400` is not failed for it. The object is deleted whatever the answers were — including, on a versioned bucket, every version the probe wrote and the delete marker itself, by id, since a plain delete there only adds a marker and would leave one noncurrent version per overwrite per restart. It is private under D48 and skipped by reconciliation (D67) if a delete ever fails. All three as above is **enforced**; a `2xx` to either of the last two is **not enforced** and is reported whenever found, ahead of any `501`, because a stored write is the outcome that is lost in silence and a backend answering `501` to one header while ignoring the other is real rather than theoretical; `501` otherwise is **unsupported** (every conditional write fails rather than being lost); `412` to the *first* overwrite is **always refused** (no conditional write can ever succeed, usually an `ETag` returned in one representation and compared in another). The last three all refuse startup, and are kept apart because the operator's diagnosis differs even where the decision does not. Any other answer is the storage error it is and refuses startup as provisioning would. It is an inherent method of the S3 adapter rather than a `Storage` method: `Storage::probe` is read-only by contract, and the `fs` backend enforces preconditions itself (D49). The client is called directly, so the check is not counted in `notedthat_storage_operations_total`. A refusal names the knowledge base, the header that was ignored, this section and the opt-out. With the opt-out, the finding is logged as `S3_CONDITIONAL_WRITES_NOT_ENFORCED`, set as `notedthat_storage_conditional_writes_enforced{kb} 0` (`1` for a bucket that enforced them), and reported by `/readyz` as a `conditional_writes` check that is `degraded` (`preconditions_not_enforced`) for the life of the process — never probed again, and never a readiness failure, since every request is still served. One writer cannot provoke a failure that only appears under contention, so a backend that passes (RustFS under lock-timeout contention) can still lose writes under load; §8.1 remains the reference for those. Garage v2.1.0 does not pass: checked against it, it stores both a mismatched `If-Match` and an `If-None-Match: *` overwrite from a single writer, and is refused. Amends §8's "without capability checks", D39 and D64. |
| D71 | Request bounds on the product listener | **The product listener bounds how long a request may take to produce its response head (`NOTEDTHAT_REQUEST_TIMEOUT_MS`, default 30 s; `NOTEDTHAT_WEBDAV_REQUEST_TIMEOUT_MS`, default 120 s, for `/webdav`), how many requests it works on at once (`NOTEDTHAT_MAX_REQUESTS_IN_FLIGHT`, default 512), and how long a connection may take to send a complete request head (`NOTEDTHAT_HEADER_READ_TIMEOUT_MS`, default 30 s).** A request past its timeout is answered `504 request_timeout` in the §6.12 envelope, with no `Retry-After`, because repeating a request that was too slow does not make it faster; a request that finds every permit taken is answered at once, never queued, with `503 backend_unavailable` and `Retry-After: 5` — the shape D38 gave every capacity refusal, because the same request will succeed once load drops — since a request waiting for a permit still holds the connection and task the cap exists to protect. A connection that has not sent a complete head in time is closed with no answer, there being no request to answer. These are distinct from the rate limiter #185 adds, which bounds how *often* a principal may ask and answers `429`: a limiter that allows N requests a second still allows unbounded concurrency if each one takes long enough. **Both request bounds end at the response head**, the span D69 times, so a large download or a streamed MCP result is not cut short, and the work worth bounding — a search's embedder call, a `PROPFIND` walk, a `PATCH` rewrite — happens before the head; how long a body takes to transfer is the proxy's business. **The two streams are exempt by construction**: the API events route and `GET /mcp` are registered on routers the bounding layer is not applied to, as are `/healthz` and `/readyz`, which answer from memory and must reach an orchestrator when the listener is full (a liveness probe refused `503` under load would restart the process for being busy). `/mcp` is routed as two legs of one service for this — `GET` bare, `POST` and `DELETE` bounded — and the bound follows from which sub-router a route is registered on rather than from a list of paths: the exempt ones are the outer router, which also carries the probes, and the API's streaming router. A route added beside the probes is therefore unbounded, which a source-level test pins rather than leaves to review. Unmatched paths are outside it too, `route_layer` not covering a router's fallback, so the cap bounds concurrent *matched* requests. **The bound runs inside authentication wherever authentication is a layer** — `/api/v1` and `/mcp`, where a bad or missing credential is `401` without drawing a permit, and `/webdav`, where it is applied inside that crate's own `basic_auth_middleware` for the same reason. The anonymous surfaces (`/browse`, `/llms.txt`, `/.well-known/…`) resolve their principal in the handler and so draw a permit before they can refuse anyone, which is what #185's limiter covers. **MCP tool calls are bounded where they do their work**: rmcp answers `POST /mcp` with an SSE response whose head comes at once, but every tool call reaches the API over loopback on this same listener, so the API's timeout and cap apply to it there and the API's `504` reaches the client as a `request_timeout` tool error. One semaphore backs every surface, so the cap is the listener's: a flood of `PROPFIND`s and a flood of searches compete for the same permits. The header-read timeout needs hyper to have a timer, which `axum::serve` does not give it, so the product listener runs its own HTTP/1 accept loop with one set — keeping upgrades and the graceful drain, and leaving the loopback metrics listener on `axum::serve`; hyper runs that clock whenever a connection waits for a head, which includes a kept-alive connection idling between requests, so it is also the idle timeout, and its default is hyper's own 30 s rather than something shorter that a proxy's upstream pool would race. Refusals are counted in `notedthat_http_requests_refused_total{surface,route,reason}` (`reason` is `timeout` or `in_flight`) and closed connections in `notedthat_http_header_read_timeouts_total`, and every refusal still counts in `notedthat_http_requests_total` under its status. Each setting refuses startup when zero or not a number, and the cap when it exceeds what a semaphore can hold (D39); unset or empty is the default. |
...
Standard AWS S3 SDK config, with one NotedThat-specific capability flag: the conditional-write check's opt-out (D70). There are no others.
...
- `NOTEDTHAT_S3_ALLOW_UNENFORCED_CONDITIONAL_WRITES` — start even when a bucket does not
  enforce the preconditions on a `PUT` (`true` or `false` exactly; default `false`, which
  refuses startup; D70). The finding stays visible for the life of the process: a log line, a
  `degraded` `/readyz` check and `notedthat_storage_conditional_writes_enforced{kb} 0`.
...
- `NOTEDTHAT_HEADER_READ_TIMEOUT_MS` — longest a connection may take to send a complete request head before it is closed; also how long an idle kept-alive connection stays open. Default `30000`.
...
| `notedthat_storage_conditional_writes_enforced` | gauge (`0`/`1`, set once at startup, `s3` only) | `kb` (D70) |
