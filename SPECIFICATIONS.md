...
- **Body.** `{"status": "ok" | "degraded" | "unavailable", "checks": {"storage": {…}, "search": {…}, "events"?: {…}}}` — the top-level word is the worst check's, so `degraded` is visible without reading the checks, and only `unavailable` costs the `200` — each check `{"backend", "status", "reason"?}` with `status` one of `ok`, `degraded`, `unavailable` and `reason` one of `timeout`, `unreachable`, `not_found` (the probed bucket or directory is gone), `disconnected` (events only). `not_found` is `degraded`, not `unavailable`: the backend answered, so the replica stays ready and `/readyz` stays `200`. The backend's own error is logged once per transition and never returned.
