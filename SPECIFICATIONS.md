...
| D20 | Bucket naming | **Slug-based, no UUID.** Deterministic: `nt-{tenant_slug}-{kb_slug}`. Idempotent from `(tenant_slug, kb_slug)` alone — no persisted id, no state lookup. DNS-safe, ≤ 63 chars (validated at KB creation per D39). See §6.6. **Amended by D57:** tenant slugs contain no hyphen, which is what makes the name injective. |
...
```
- `tenant_slug`: 1–20 chars, `[a-z0-9]`, **no hyphen** (D57)
- `kb_slug`: 1–40 chars, `[a-z0-9-]`, no leading/trailing hyphen (§6.8)
- Injective: because the tenant slug has no hyphen, the first hyphen after `nt-` always ends the tenant, so two distinct `(tenant_slug, kb_slug)` pairs never share a bucket name — or, under the `fs` backend, a directory (D49, D57).
...
2. Validate every slug (`[a-z0-9-]{1,40}`, no leading/trailing hyphen; the tenant slug is `[a-z0-9]{1,20}` per D57; reject empty display names). Reject any `(tenant_slug, kb_slug)` whose derived bucket name (§6.6) exceeds 63 chars.
