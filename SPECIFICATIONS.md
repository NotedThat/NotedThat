...
└── crates/
    ...
    ├── notedthat-events/         # object change event log adapters (D54): memory ring, NATS JetStream
...
| `GET` | `/readyz` | Readiness — unauthenticated, unversioned. `503` while a configured event broker is disconnected (D54); S3 and Qdrant are not probed in v1 |
...
| `GET` | `/api/v1/knowledgebases/{kb_slug}/events` | Object change events as SSE (D54, §6.14). Query params: `prefix`, `event`, `mime`; header `Last-Event-ID`. `404` unless an events backend is configured. Like `search`, a literal sibling of the object catch-all |
...
### 6.14 Object change events `[DECIDED — D54]`
...
- **MCP**: `subscribe`/`listChanged` Resources capability (post-v1) — can be fed by the D54 event stream; per-KB access control for Resources.
