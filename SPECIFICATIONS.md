...
- **MCP**: binding a session to the principal that opened it and budgeting sessions per principal rather than per process (D66 leaves rmcp's sessions unbound, so another authenticated caller holding a session id can read its notification stream, and one caller can hold all 256 slots) — [#179](https://github.com/NotedThat/NotedThat/issues/179); a `NOTEDTHAT_MCP_MAX_SESSIONS` knob for the D66 constant.
