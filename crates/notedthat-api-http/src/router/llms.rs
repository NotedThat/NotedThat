...
Start by sending `GET /api/v1/knowledgebases` to list the knowledge bases available to the caller. Each entry carries `kb_slug`, `display_name` and, when the operator set one, a `description` of what that knowledge base is for; read the descriptions before choosing where to search. Send `GET /api/v1/knowledgebases/{kb_slug}` to list objects in one knowledge base. Read an object with `GET /api/v1/knowledgebases/{kb_slug}/{path}`. Search with `POST /api/v1/knowledgebases/{kb_slug}/search` and a JSON search body. If results look incomplete or out of date, `GET /api/v1/knowledgebases/{kb_slug}/index` reports the knowledge base's index state (`healthy`, `indexing`, `backpressured`, `stale` or `failed`) and its last failure.
...
`POST /mcp` is a Model Context Protocol server over the streamable HTTP transport with sessions: `initialize` returns an `Mcp-Session-Id` that every later request carries, answers are SSE-framed, `GET /mcp` is the notification leg and `DELETE /mcp` ends the session; the legacy `/sse` paths answer `405`. With an events backend the server advertises `resources.subscribe` and `resources.listChanged`, and `resources/subscribe` on a `notedthat://` URI yields `notifications/resources/updated` on every write or delete of that object. It offers ten tools — `list_knowledgebases`, `search`, `read`, `list`, `write`, `edit`, `append`, `replace`, `move`, `delete` — and `notedthat://` resources. `search` takes `kb` (an array of slugs; omit it to search every knowledge base the caller may see), `query`, `filters` — the same seven fields as the HTTP body's `filter` — and a per-knowledge-base `limit`. Every tool call runs as the caller, bound by the access rules above; a denial surfaces as a tool error (`forbidden` for a signed-in caller, `not_found` for an anonymous one), never as a partial result.
