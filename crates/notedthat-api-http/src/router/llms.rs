...
Start by sending `GET /api/v1/knowledgebases` to list the knowledge bases available to the caller. Each entry carries `kb_slug`, `display_name` and, when the operator set one, a `description` of what that knowledge base is for; read the descriptions before choosing where to search. Send `GET /api/v1/knowledgebases/{kb_slug}` to list objects in one knowledge base. Read an object with `GET /api/v1/knowledgebases/{kb_slug}/{path}`. Search with `POST /api/v1/knowledgebases/{kb_slug}/search` and a JSON search body.
