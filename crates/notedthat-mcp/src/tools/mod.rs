...
mod resources_shared {
    ...
    /// `tools/list` must return exactly the 7 M7 tools, unchanged.
    ///
    /// Uses `get_tool()` (generated by `#[tool_handler]` from the same router that powers
    /// `list_tools`) to verify each expected tool is present without creating a full
    /// `RequestContext`.
    #[test]
    fn tools_list_returns_exactly_seven_m7_tools() {
        let h = handler("http://localhost:8080");
        let m7 = [
            "list_knowledgebases",
            "search",
        ...
        for name in m7 {
            assert!(
                ...
                "expected M7 tool {name:?} to be registered"
            );
        }
        assert!(
            h.get_tool("nonexistent_tool").is_none(),
            "no extra tools must appear beyond the 7 M7 tools"
        );
    }
